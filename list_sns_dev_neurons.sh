...
for DEV_IDENT in "$HOME"/.config/dfx/identity/dev-ident-*; do
  ...
  export NEURON_IDS="$(dfx canister --network "${NETWORK}" call "${SNS_GOVERNANCE_CANISTER_ID}" list_neurons "(record {of_principal = opt principal\"${DX_PRINCIPAL}\"; limit = 0})")"
...
dfx canister --network "${NETWORK}" call "${SNS_GOVERNANCE_CANISTER_ID}" list_neurons "(record {of_principal = opt principal\"$CANISTER_DEV_NEURON\"; limit = 0})"
